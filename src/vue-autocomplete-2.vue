    ...
    <div class="g-autocomplete" :class="{'disabled':disabled}" :tabindex="disabled?'':1">
        ...
        <div class="selected-value" v-bind:class="{'dropdown-opened':dropdownOpened}">
            <input type="text" v-model="filter" ref="text" class="text"
                   v-on:keydown="filterKeydown($event)" v-on:blur="lostFocus"/>
        </div>
    ...
    export default {
        ...
        methods:{
            ...
            setFilter:function () {
                var self = this;
                if (self.populate){
                    if (!self.changeType.startsWith('valueChanged')){
            ...
            handleKeyDown:function (event) {
                if (!this.dropdownOpened && event.code!='Tab'){
                    ...
                    if (this.focusedIndex && this.filteredOptions[this.focusedIndex]!==undefined){
