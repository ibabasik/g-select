<template>
	<div class="g-select" :class="{'disabled':disabled}" tabindex="1" @keydown="handleKeyDown($event)"
	     @blur="lostFocus">
		<div v-on:click="toggleDropdown($event)" class="selected-value"
		     v-bind:class="{'dropdown-opened':dropdownOpened}">
			<span class="placeholder"
			      v-if="placeholder && (!rawSelectedValue || (multiple && !rawSelectedValue.length)) ">{{placeholder}}</span>
		...
		<div v-if="dropdownOpened" class="dropdown" ref="dropdown">
			...
			<div class="options-container" ref="options"
			     :style="actualMaxDropdownHeight?{'max-height':actualMaxDropdownHeight}:''">
...
<script>
	...
	export default {
		...
		created: function () {
			...
			this.setSelectedOptionType(true);
		...
		computed: {
			...
			selectedValue: function () {
				if (this.multiple && this.isComplex) {
					return this.rawSelectedValue && this.bindId ? _.map(this.rawSelectedValue, function (v) {
						return v[this.idPath];
					}.bind(this)) : this.rawSelectedValue;
				} else {
